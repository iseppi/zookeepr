...
${ h.hidden('fp_timestamp', c.payment.creation_timestamp_utc.strftime('%Y%m%d%H%M%S')) }
...
${ h.hidden('callback_url', h.url_for(protocol='http', qualified=True, controller='payment', action='new', id=c.payment.id, payment_id=c.payment.id, invoice_id=c.payment.invoice_id)) }
